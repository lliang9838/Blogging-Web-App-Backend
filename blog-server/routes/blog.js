...
router.get("/:username", function (req, res, next) {
  ...
  if (typeof start !== "undefined") {
    //this is the correct way to check if something is undefined
  ...
  cursor.toArray(function (err, docs) {
    ...
    for (
      let i = idx;
      i < Math.min(5 + idx, docs.length);
      i++ //this is the converting part as well, only pushes the first 5 for now
    ) {
      var obj = {
        title: writer.render(reader.parse(docs[i].title)),
        body: writer.render(reader.parse(docs[i].body)),
      };
